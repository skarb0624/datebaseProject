...
import io.mobile.sales.order.Order;
...
public class DLCService {
    ...
    public static DLC selectById(final int gameNo) {

        ResultSet rs = null;
        PreparedStatement psmtQuery = null;

        try (Connection conn = DriverManager.getConnection(Conf.DB_URL, Conf.DB_USER, Conf.DB_PASSWORD)) {
            String query = "SELECT D.*, game_number FROM game G, DLC D "
                    + "WHERE G.game_number = D.game_number AND D.game_number = ?";
            psmtQuery = conn.prepareStatement(query);
